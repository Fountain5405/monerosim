...
- **`patches/monero-sim-peerlist-dump.patch`** (`--peerlist-dump-file`, off by
  ...
  `./setup.sh --hardfork` now applies every vendored patch into `monerod-hf`
  (per-patch tripwire + provenance); the primary `monerod` stays vanilla. The
  eclipse scenarios run only their dumping nodes on `monerod-hf`. Rationale in
  `docs/PEERLIST_DUMP_PATCH.md`.
