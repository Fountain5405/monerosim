...
fn get_node_as_number(gml_node: &GmlNode) -> Option<String> {
    gml_node
        .attributes
        .get("AS")
        .or_else(|| gml_node.attributes.get("as"))
        .cloned()
}
...
pub fn get_agent_ip(
    ...
    if let Some(pre_registered) = ip_registry.get_ip_for_agent(agent_id).cloned() {
        log::debug!(
            "Using pre-registered IP {} for agent {}",
            pre_registered,
            agent_id
        );
    ...
    if let Some(group) = subnet_group {
        match ip_registry.assign_subnet_group_ip(group, agent_id) {
            Ok(ip) => {
                log::info!(
                    "Assigned subnet group IP {} to agent {} (group: {})",
                    ip,
                    agent_id,
                    group
                );
            ...
            if let Some(gml_node) = gml.nodes.iter().find(|node| node.id == network_node_id) {
                ...
                if let Some(pre_allocated_ip) = gml_node.get_ip() {
                    ...
                    if !GmlNode::is_valid_ip(pre_allocated_ip) {
                        log::warn!(
                            "Invalid pre-allocated IP '{}' for node {} in GML file",
                            pre_allocated_ip,
                            network_node_id
                        );
                    } else {
                        // Check for conflicts with existing assignments
                        if let Some(conflicting_agent) =
                            ip_registry.get_agent_for_ip(pre_allocated_ip)
                        {
                            if conflicting_agent != agent_id {
                                ...
                                log::debug!(
                                    "Using pre-allocated IP {} for agent {} (node {})",
                                    pre_allocated_ip,
                                    agent_id,
                                    network_node_id
                                );
                            ...
                            if let Err(conflict) =
                                ip_registry.register_pre_allocated_ip(pre_allocated_ip, agent_id)
                            {
                                log::error!(
                                    "Failed to register pre-allocated IP {} for agent {}: {}",
                                    pre_allocated_ip,
                                    agent_id,
                                    conflict
                                );
                                ...
                                log::info!(
                                    "Assigned pre-allocated IP {} to agent {} (node {})",
                                    pre_allocated_ip,
                                    agent_id,
                                    network_node_id
                                );
                    ...
                    if let Some(as_ip) = subnet_manager.assign_as_aware_ip(&as_number) {
                        ...
                        if let Some(conflicting_agent) = ip_registry.get_agent_for_ip(&as_ip) {
                            if conflicting_agent != agent_id {
                                ...
                                log::debug!(
                                    "Using AS-aware IP {} for agent {} (AS {}, node {})",
                                    as_ip,
                                    agent_id,
                                    as_number,
                                    network_node_id
                            ...
                            // Register this IP in our central registry
                            if let Err(conflict) =
                                ip_registry.register_pre_allocated_ip(&as_ip, agent_id)
                            {
                                log::warn!(
                                    "Failed to register AS-aware IP {} for agent {}: {}",
                                    as_ip,
                                    agent_id,
                                    conflict
                                );
                ...
                log::warn!(
                    "Agent {} assigned to node {} which doesn't exist in GML topology",
                    agent_id,
                    network_node_id
                );
            }
    ...
    match ip_registry.assign_ip(agent_type, agent_id) {
        Ok(ip) => {
            log::info!(
                "Assigned dynamic IP {} to agent {} using global registry",
                ip,
                agent_id
            );
            Ok(ip)
        ...
        Err(error) => {
            ...
            log::warn!(
                "IP registry assignment failed for {}: {}. Using geographic fallback.",
                agent_id,
                error
            );
