...
/// Helper function to create a Python script command with proper exit code handling
fn create_python_script_command(current_dir: &str, script_name: &str) -> String {
    format!(
        "-c 'cd {} && source venv/bin/activate && python3 scripts/{} || exit $?'",
...
pub fn generate_shadow_config(config: &Config, output_dir: &Path) -> color_eyre::eyre::Result<()> {
    ...
    // Add block controller script (Python version)
    let block_controller_process = ShadowProcess {
        path: "/bin/bash".to_string(),
        args: create_python_script_command(&current_dir, "block_controller.py"),
    ...
    let simple_test_process = ShadowProcess {
        path: "/bin/bash".to_string(),
        args: create_python_script_command(&current_dir, "simple_test.py"),
        environment: environment.clone(),
        start_time: "120s".to_string(), // Start after block controller
    };
    ...
    let sync_check_process = ShadowProcess {
        path: "/bin/bash".to_string(),
        args: create_python_script_command(&current_dir, "sync_check.py"),
        environment: environment.clone(),
        start_time: "300s".to_string(), // Start after some blocks are generated
    };

    let sync_check_host = ShadowHost {
        network_node_id: 0,
        ip_addr: None,
        processes: vec![sync_check_process],
    };

    hosts.insert("sync-check".to_string(), sync_check_host);

    // Add monitor script
    let monitor_process = ShadowProcess {
        path: "/bin/bash".to_string(),
        args: create_python_script_command(&current_dir, "monitor.py"),
        environment: environment.clone(),
        start_time: "30s".to_string(), // Start early to monitor the whole simulation
    };
    ...
    // Add P2P connectivity test script
    let p2p_test_process = ShadowProcess {
        path: "/bin/bash".to_string(),
        args: create_python_script_command(&current_dir, "test_p2p_connectivity.py"),
        environment: environment.clone(),
        start_time: "90s".to_string(), // Start after nodes are up
    };

    let p2p_test_host = ShadowHost {
        network_node_id: 0,
        ip_addr: None,
        processes: vec![p2p_test_process],
    };
