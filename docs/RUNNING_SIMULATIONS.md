...
```bash
./target/release/monerosim --config monerosim.yaml --output shadow_output
```
...
A simulation with the default `monerosim.yaml` configuration (8h simulation time, 25 agents) takes several hours of wall-clock time depending on your hardware.
...
Using the default `monerosim.yaml` configuration:
