...
| 1.7 | **Retry `get_info()` in `BaseAgent.setup()`** (NEW — discovered during 400-node calibration; `AutonomousMinerAgent` crashes on transient empty-error from `get_info()` at sim t=15s) | TODO | — |
