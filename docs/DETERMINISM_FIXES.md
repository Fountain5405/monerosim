...
**Files affected:**
- `agents/miner_distributor/` (package)
- `agents/regular_user.py`

**Issue:** These agents used Python's `random` module without seeding from `SIMULATION_SEED`:
```python
# miner_distributor - No seeding
random.choice()
random.uniform()
...
**Additional issue:** miner_distributor also uses `np.random.choice()` - numpy's RNG is separate from Python's `random` module and requires its own seeding via `np.random.seed()`.
...
**File:** `src/orchestrator.rs` (historical)
```rust
...
**Fix:** Use a deterministic identifier based on `simulation_seed` or timestamp from config. (Resolved: the GML now goes to `<output_dir>/topology.gml`, no PID involved.)

---

### 3. Python `hash()` Function Non-determinism (HIGH)

**File:** `agents/autonomous_miner.py`
```python
self.agent_seed = self.global_seed + hash(agent_id)
```
...
**Files affected:**
- `agents/agent_discovery.py`
- `agents/simulation_monitor/`
- `agents/miner_distributor/`
- `agents/dns_server.py`
...
**File:** `agents/monero_rpc.py::wait_until_ready`
...
|---|-------|----------|----------|---------|----------------|
| 1 | Unseeded RNG (miner_distributor) | CRITICAL | Python RNG | miner_distributor/ | Easy ✅ |
| 2 | Unseeded RNG (regular_user) | CRITICAL | Python RNG | regular_user.py | Easy ✅ |
| 3 | Process ID in temp path | CRITICAL | Rust runtime | orchestrator.rs | Easy ✅ |
| 4 | Python hash() randomization | HIGH | Environment | All Python agents | Easy ✅ |
| 5 | HashMap iteration order | MEDIUM-HIGH | Rust collections | orchestrator.rs, distribution.rs | Medium ✅ |
| 6 | time.time() usage | MEDIUM | Python timing | Multiple agents | N/A (Shadow intercepts) |
| 7 | File system race conditions | MEDIUM | Concurrency | /tmp/monerosim_shared | Medium ✅ |
| 8 | Monero internal PRNG | PARTIAL | Shadow | shadowformonero/injector.c | RNG seeded ✅, threading causes ~4% variance |
| 9 | DNS server file access | MEDIUM | External | dns_server.py | Easy ✅ |
| 10 | RPC retry jitter | LOW-MEDIUM | Timing | monero_rpc.py | N/A (Shadow intercepts) |
| 11 | Current directory dependency | LOW | Environment | wallet.rs | Easy (low priority) |
| 12 | Shadow seed not set | CRITICAL | Shadow config | orchestrator.rs, types.rs | Easy ✅ |
...
2. ✅ Add RNG seeding to miner_distributor and regular_user.py
...
8. ✅ Add file locking to autonomous_miner.py for agent_registry.json access

### Phase 5: Shadow RNG Seeding - COMPLETED
9. ✅ Pass simulation_seed to Shadow's `general.seed` config (CRITICAL)
...
10. ✅ Fix Shadow's injector constructor priority to run before monerod's RNG initialization
...
12. Verify full determinism with run comparisons after Phase 6 fix
...
**Python files:**
- `agents/miner_distributor/` - Add RNG seeding
